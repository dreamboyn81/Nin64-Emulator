...
{
    static int task_log_count;

    if(task_log_count<8)
    {
        print("hw_sp_start task %08X type=%04X flags=%04X data=%08X/%04X boot=%08X/%04X\n",
            sptaskpos,
            sptask.type,
            sptask.flags,
            sptask.m_data_ptr,
...
void hw_sp_dmawrite(void)
{
    ...
    if(dma_log_count<8)
    {
        print("hw_sp_dmawrite from=%08X to=%08X len=%08X\n",from,to,cnt);
        dma_log_count++;
    }
...
//    print("hw: memio (pc=%08X,memio=%08X,spstat=%08X)\n",st.pc,st.memiodetected,RSP[4]);
    ...
    if(WVI[1] || WVI[2])
    {
        static int vi_log_count;
        dword origin=WVI[1];
        dword width=WVI[2];

        if(vi_log_count<12)
        {
            print("hw_vi_write origin=%08X width=%08X status=%08X thread=%i pc=%08X\n",
                origin,
                width,
                WVI[0],
                st.thread,
...
int hw_ismemiorange(dword addr)
{
    ...
    if(addr==0x04400000) return(1); // VI registers are handled
