...
void p_osSpTaskStartGo(void)
{
    ...
    if(logged<8)
    {
        print("osSpTaskStartGo patch call %i at pc=%08X arg=%08X sptaskload=%i\n",
            logged+1,st.pc,A0.d,st2.sptaskload);
        logged++;
    }
...
void p_osSpTaskLoad(void)
{
    ...
    if(logged<8)
    {
        print("osSpTaskLoad patch call %i at pc=%08X arg=%08X\n",
            logged+1,st.pc,A0.d);
        logged++;
    }
...
{
    print("osCreateViManager(%08X,%08X,%08X)\n",A0.d,A1.d,A2.d);
}
