...
void addi64(dword *d,dword *a,dword *b)
{
    ...
    av=((qword)a[1]<<32)|(qword)a[0];
    bv=((qword)b[1]<<32)|(qword)b[0];
    res=av+bv;
    d[0]=(dword)res;
    d[1]=(dword)(res>>32);
...
static dword op_memaddr(dword opcode)
{
    dword a;
    a = OP_IMM(opcode);
    a = SIGNEXT16(a);
...
{
    qint product=(qint)a*(qint)b;
    st.mlo.d=(dword)product;
    st.mhi.d=(dword)(product>>32);
}

static void op_multu(int a,int b)
{
    qword product=(qword)(dword)a*(qword)(dword)b;
    st.mlo.d=(dword)product;
    st.mhi.d=(dword)(product>>32);
}
...
static void op_divu(int a,int b)
{
    ...
    st.mlo.d=(dword)a/(dword)b;
    st.mhi.d=(dword)a%(dword)b;
}
...
static void op_branch(dword opcode,int doit,int likely,int link)
{
    if(doit)
    {
        dword imm=(dword)SIGNEXT16(OP_IMM(opcode));
        if(link)
        { // link
            st.g[31].d=st.pc+8;
            st.branchtype=BRANCH_CALL;
