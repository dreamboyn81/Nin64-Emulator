...
static void sym_print_patch_status(const char *name)
{
    int i;

    for(i=0;oscall[i][15];i++)
    {
        const char *entry=(const char *)oscall[i][15];
        if(memicmp(entry,name,strlen(name)))
        {
            continue;
        }
        ...
        if(oscall[i][12])
        {
            print("patch status: %-18s FOUND at %08X patch %2i class %i\n",
                name,
                oscall[i][12],
                oscall[i][14],
                oscall[i][13]);
        ...
        {
            print("patch status: %-18s missing (maybe %08X) patch %2i\n",
                name,
                oscall[i][11],
                oscall[i][14]);
        }
        ...
        {
            print("patch status: %-18s missing patch %2i\n",
                name,
                oscall[i][14]);
        }
        return;
    ...
    print("patch status: %-18s unknown\n",name);
}

static void sym_print_runtime_patch_summary(void)
{
    sym_print_patch_status("osInitialize");
    sym_print_patch_status("osCreateThread");
    sym_print_patch_status("osStartThread");
    sym_print_patch_status("osSetThreadPri");
    sym_print_patch_status("osCreateMesgQueue");
    sym_print_patch_status("osSendMesg");
    sym_print_patch_status("osRecvMesg");
    sym_print_patch_status("osSetEventMesg");
    sym_print_patch_status("osCreateViManager");
    sym_print_patch_status("osSpTaskLoad");
    sym_print_patch_status("osSpTaskStartGo");
    sym_print_patch_status("osViSetEvent");
    sym_print_patch_status("osViSwapBuffer");
    sym_print_patch_status("osAiSetNextBuffer");
}
...
#ifdef __ANDROID__
    sym_print_runtime_patch_summary();
#endif
}
...
//        if(sym[i].text[0]=='o' && sym[i].text[1]=='s' && sym[i].text[2]>='A')
        if(sym[i].addr<0x20e000)
        {
            ...
            oscall[n][15]=(uintptr_t)sym[i].text;
