...
{
    static int queue_create_log_count;
    int id,i;
    ...
    if(queue_create_log_count<24)
    {
        print("queue_create id=%02i addr=%08X size=%i thread=%i ra=%08X\n",
            id,m_queue,size,st.thread,RA.d);
        queue_create_log_count++;
    }
...
{
    static int send_log_count[MAXQUEUE];
    ...
    if((id==1 || id==2 || id==12 || id==14) && send_log_count[id]<8)
    {
        print("queue_send q=%02i msg=%08X block=%i thread=%i pc=%08X ra=%08X used=%i/%i\n",
            id,m_mesg,block,st.thread,st.pc,RA.d,q->num,q->size);
        send_log_count[id]++;
    }
...
{
    static int recv_log_count[MAXQUEUE];
    dword id=mem_read32(m_queue);
    OSQueue *q=queue+id;
    int i,m_mesg,pos;
    ...
    if((id==1 || id==2 || id==12 || id==14) && recv_log_count[id]<8)
    {
        print("queue_recv q=%02i block=%i thread=%i pc=%08X ra=%08X used=%i/%i\n",
            id,block,st.thread,st.pc,RA.d,q->num,q->size);
        recv_log_count[id]++;
    }
...
{
    static int event_log_count[MAXEVENTS];
    int qid,limit;
    if(ev>=MAXEVENTS)
    {
    ...
    if(qid)
    {
        if((ev==OS_EVENT_SP || ev==OS_EVENT_DP || ev==OS_EVENT_RETRACE || ev==OS_EVENT_PI)
        && event_log_count[ev]<8)
        {
            print("os_event(%02i) -> queue %02i mesg=%08X thread=%i pc=%08X ra=%08X used=%i/%i\n",
                ev,
                qid,
                event[ev].mesg,
                st.thread,
...
void os_dumpinfo(void) // osinfo
{
    ...
    for(i=1;i<MAXTHREAD;i++) if(thread[i].active)
    {
        ...
        print("\n");
        print("           a0=%08X a1=%08X a2=%08X ra=%08X\n",
            thread[i].st.g[4].d,
            thread[i].st.g[5].d,
            thread[i].st.g[6].d,
            thread[i].st.g[31].d);
...
//    print("#dma %08X->%08X (%04X)\n",devaddr,vaddr,nbytes);
    ...
    if(pi_dma_log_count<24)
    {
        print("osPiStartDma dev=%08X vaddr=%08X bytes=%i queue=%08X thread=%i ra=%08X\n",
            devaddr,vaddr,nbytes,m_msgqueue,st.thread,RA.d);
        pi_dma_log_count++;
    }
