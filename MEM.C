...
void mem_init(int ramsize)
{
    ...
    mem.ram=(byte *)( (((uintptr_t)mem.ramalloc+8192u) & ~(uintptr_t)4095u) );
...
{
    byte *b;
    b=(byte *)( (uintptr_t)data - (uintptr_t)dst );
    switch(rw)
    {
    case MAP_W:
...
void mem_mapcopy(dword dst,int rw,dword src)
{
    byte *br;
    byte *bw;
    br=(byte *)((uintptr_t)mem.lookupr[mempage(src)]-(uintptr_t)dst+(uintptr_t)src);
    bw=(byte *)((uintptr_t)mem.lookupw[mempage(src)]-(uintptr_t)dst+(uintptr_t)src);
    switch(rw)
    {
    case MAP_W:
...
dword lookupvalue(byte **x,int i)
{
    ...
    a=((uintptr_t)x[i]+(uintptr_t)(i*4096))-(uintptr_t)mem.ram;
...
//            print("- %08X\n",a);
            if(i>cnt) break;
            if(a!=0xffffffff) x[i]=(byte *)((uintptr_t)mem.ram+(uintptr_t)a-(uintptr_t)(i*4096));
            i++;
            break;
        case 2:
            fread(&n,1,4,f1);
                ...
                */
                {
                    if(m!=0xffffffff)
                    {
                        for(j=0;j<n;j++) lookup[i+j]=mem.ram+m-(i+j)*4096;
                    }
                }
                i+=n;
            }
